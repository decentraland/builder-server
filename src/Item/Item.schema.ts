...
const baseItemSchema = Object.freeze({
  properties: {
    ...
    thumbnail: { type: 'string', maxLength: 1000, pattern: '^[^:]*$' },
    video: { type: ['string', 'null'], maxLength: 1000, pattern: '^[^:]*$' },
    eth_address: { type: 'string' },
    collection_id: { type: ['string', 'null'], format: 'uuid' },
    blockchain_item_id: { type: ['string', 'null'] },
    price: { type: ['string', 'null'], maxLength: 78, pattern: '^[0-9]*$' },
    beneficiary: {
      type: ['string', 'null'],
      pattern: `^(${matchers.address})?$`,
    },
    rarity: {
      type: ['string', 'null'],
    ...
    utility: { type: ['string', 'null'], maxLength: 64, pattern: '^[^:]*$' },
